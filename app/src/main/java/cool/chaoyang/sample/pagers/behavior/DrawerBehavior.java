...
public class DrawerBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
  ...
  private int gestureInsetHorizontal;
  private boolean resetRequestDisallowInterceptTouchEvent;

  /** Callback for monitoring events about bottom sheets. */
  public abstract static class DrawerCallback {
  ...
  private int peekWidthGestureInsetBuffer;
...
//  private int childHeight;
  ...
  @Nullable WeakReference<CoordinatorLayout> coordinatorLayoutRef;
  ...
  public boolean onLayoutChild(
    ...
    if (coordinatorLayoutRef == null){
      coordinatorLayoutRef = new WeakReference<>(parent);
    }
...
//    childHeight = child.getHeight();
    ...
    fitToContentsOffset = max(0, parentWidth - childWidth);
  ...
  public boolean onInterceptTouchEvent(
    ...
    boolean b = action == MotionEvent.ACTION_MOVE
            && scroll != null
            && !ignoreEvents
  ...
  private void requestParentDisallowInterceptTouchEvent(int action,boolean b) {
    ...
    CoordinatorLayout coordinatorLayout = coordinatorLayoutRef.get();
...
//      return false;
  ...
  public void onNestedScrollAccepted(@NonNull CoordinatorLayout coordinatorLayout, @NonNull V child, @NonNull View directTargetChild, @NonNull View target, int axes, int type) {
    Log.d(TAG, "onNestedScrollAccepted() called with: coordinatorLayout = [" + coordinatorLayout + "], child = [" + child + "], directTargetChild = [" + directTargetChild + "], target = [" + target + "], axes = [" + axes + "], type = [" + type + "]");
  ...
  public void onStopNestedScroll(
      ...
      @NonNull View target,
      int type) {
    Log.d(TAG, "onStopNestedScroll() called with: coordinatorLayout = [" + coordinatorLayout + "], child = [" + child + "], target = [" + target + "], type = [" + type + "]");
...
//        targetState = STATE_COLLAPSED;
    ...
    startSettlingAnimationLeft(child, targetState, left, false);
    nestedScrolled = false;
  }
  ...
  public final void setPeekHeight(int peekHeight, boolean animate) {
    ...
    } else if (peekHeightAuto || this.peekWidth != peekHeight) {
      peekHeightAuto = false;
      this.peekWidth = max(0, peekHeight);
      layout = true;
    }
    // If sheet is already laid out, recalculate the collapsed offset based on new setting.
    // Otherwise, let onLayoutChild handle this later.
  ...
  public int getPeekWidth() {
    return peekHeightAuto ? PEEK_HEIGHT_AUTO : peekWidth;
  }
   ...
   * @deprecated use {@link #addDrawerCallback(DrawerCallback)} and {@link
   *     #removeDrawerCallback(DrawerCallback)} instead
   */
  @Deprecated
  public void setBottomSheetCallback(DrawerCallback callback) {
    Log.w(
        TAG,
   ...
   * @param callback The callback to notify when bottom sheet events occur.
   */
  public void addDrawerCallback(@NonNull DrawerCallback callback) {
   ...
   */
  public void removeDrawerCallback(@NonNull DrawerCallback callback) {
  ...
  private int calculatePeekWidth() {
    if (peekHeightAuto) {
      int desiredWidth = max(peekWidthMin, parentWidth - parentWidth * 9 / 16);
      return min(desiredWidth, childWidth);
    }
    if (!gestureInsetBottomIgnored && gestureInsetBottom > 0) {
      return max(peekWidth, gestureInsetHorizontal + peekWidthGestureInsetBuffer);
    }
    return peekWidth;
  }
  ...
  private void calculateCollapsedOffset() {
    ...
    if (fitToContents) {
      collapsedOffset = max(parentWidth - peek, fitToContentsOffset);
    } else {
  ...
  boolean shouldHide(@NonNull View child, float yvel) {
    ...
    int peek = calculatePeekWidth();
    final float newTop = child.getLeft() + yvel * HIDE_FRICTION;
      ...
      ViewUtils.doOnApplyWindowInsets(
          ...
          new ViewUtils.OnApplyWindowInsetsListener() {
            ...
            public WindowInsetsCompat onApplyWindowInsets(
              ...
              gestureInsetHorizontal = Math.max(insets.getMandatorySystemGestureInsets().right, insets.getMandatorySystemGestureInsets().left);
  ...
  void settleToState(@NonNull View child, int state) {
    ...
    startSettlingAnimationLeft(child, state, left, false);
  }
...
//  }
  ...
  private final ViewDragHelper.Callback dragCallback =
      new ViewDragHelper.Callback() {
        ...
        ///松手隐藏
        private boolean releasedToHide(@NonNull View child) {
          // Needs to be at least half way to the bottom.
          return true|| child.getLeft() > (parentWidth - getExpandedOffset()) / 2;
        }
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
          ...
          if (xvel < 0) { // Moving up
            ...
            if ((Math.abs(xvel) > Math.abs(yvel) && xvel > SIGNIFICANT_VEL_THRESHOLD)
                || releasedToHide(releasedChild)) {
              left = parentWidth;
              targetState = STATE_HIDDEN;
            ...
            // settle to the nearest correct position.
            int currentLeft = releasedChild.getLeft();
            if (fitToContents) {
...
//              targetState = STATE_COLLAPSED;
          ...
          startSettlingAnimationLeft(releasedChild, targetState, left, true);
        }
  ...
  protected static class SavedState extends AbsSavedState {
    ...
    public SavedState(Parcelable superState, @NonNull DrawerBehavior<?> behavior) {
      super(superState);
      this.state = behavior.state;
      this.peekHeight = behavior.peekWidth;
    ...
    /**
     * This constructor does not respect flags: {@link DrawerBehavior#SAVE_PEEK_HEIGHT}, {@link
     * DrawerBehavior#SAVE_FIT_TO_CONTENTS}, {@link DrawerBehavior#SAVE_HIDEABLE}, {@link
     * DrawerBehavior#SAVE_SKIP_COLLAPSED}. It is as if {@link DrawerBehavior#SAVE_NONE}
     * were set.
     *
     * @deprecated Use {@link #SavedState(Parcelable, DrawerBehavior)} instead.
     */
    @Deprecated
  ...
  /**
   * A utility function to get the {@link DrawerBehavior} associated with the {@code view}.
   *
   * @param view The {@link View} with {@link DrawerBehavior}.
   * @return The {@link DrawerBehavior} associated with the {@code view}.
   */
  @NonNull
  @SuppressWarnings("unchecked")
  public static <V extends View> DrawerBehavior<V> from(@NonNull V view) {
    ...
    return (DrawerBehavior<V>) behavior;
  }
